...
def main_page():
    if "username" in session:
        ...
        return render_template('index.html', username=username)   # 저장된 username 변수를 main.html 페이지로 전달, 즉 유저마다 각기다른 /user페이지를 보게됨
    else:  # 세션이 존재하지 않는 경우
...
def my_profile():
    if "username" in session:
        ...
        # 로그인을 했다면(세션안에 정보가 있으니) 로그인하여 생성된 세션 딕셔너리의 값을 변수 username에 저장
        return render_template('profile.html', username=username)   # 저장된 username 변수를 profile.html 페이지로 전달, 즉 유저마다 각기다른 /user페이지를 보게됨
    else:  # 세션이 존재하지 않는 경우
        return redirect(url_for("login_btn"))  # 세션안에 정보가 없어서(혹은 브라우저를 나가면 세션이 삭제) 로그인 페이지로 redirect
...
def none1_page():
    if "username" in session:
        username = session["username"]  # 로그인을 했다면(세션안에 정보가 있으니) 로그인하여 생성된 세션 딕셔너리의 값을 변수 username에 저장
        pw = session['pw']
        return render_template('none1.html', username=username, pw=pw)   # 저장된 username 변수를 main.html 페이지로 전달, 즉 유저마다 각기다른 /user페이지를 보게됨
    else:  # 세션이 존재하지 않는 경우
        return redirect(url_for("login_btn"))  # 세션안에 정보가 없어서(혹은 브라우저를 나가면 세션이 삭제) 로그인 페이지로 redirect
...
def login_btn():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        if account:
            ...
            session['pw'] = account[4]
            print(account[0], account[1], account[2], account[4])
            msg = '로그인 성공'
            session.permanent = True   # 세션 작동
            return render_template('/index.html', username=username, msg=msg)
        else:
            msg = '로그인 실패!'
